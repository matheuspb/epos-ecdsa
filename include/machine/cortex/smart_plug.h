...
public:
    ...
    // Calculate the average electrical power, that is, the active power, of all the instant power values
    // in a period of time, represented by a sinusoidal signal. This is calculated using the following equation: 
    // integrate (from 0 to T) { v[t]*i[t]*dt }
    unsigned int average() {
        int j;
        long i_avg = 0, ref, p_avg = 0, v[N], v_shift[N], i[N], p[N]; //i_rms = 0
        ...
        // Remove the 11 first voltage samples, that aren't properly reconstructed
        for(j = N_ERR; j < N; j++) {
            p_avg += p[j];
        }
...
public:
    ...
    static unsigned int power(unsigned int dev) {
        assert(dev < 2);
