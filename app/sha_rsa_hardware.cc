...
int main()
{
    ...
    print(hash);
    uint8_t* hashbig = convertToBigEndian(hash);
	HWBignum plaintext = HWBignum((uint32_t*)hashbig, 32);
...
//	KeyPair kp = KeyPair(nbig, skbig,pk);
	...
	KeyPair kp = KeyPair();
	cout << "Trying to generate RSA key pair of size: " << (KeyPair::_320BITS*32)*2 << "bits.." << endl;
	kp.generateKeyPair(KeyPair::_320BITS);
	...
	cout << "Ecrypted SHA:" <<endl;
	cifrado.printResult(cout);

	Number decrypted = rsa.decrypt(kp.getPublicKey(), cifrado);
	cout << "Decrypted SHA:" <<endl;
	decrypted.printResult(cout);
	if(decrypted == plaintext)
		cout << "Decrypted and plaintext are the same." << endl;
