...
int main()
{
  ...
  cout << "Fixing input variations from PUF: " << endl;
  bch->repair(&sample_def, block, input_size, block + input_size);

  cout << "Check after repair: " << endl;
  test = bch->verify(&sample_def, block, input_size, block + input_size);
  cout << (test == 0 ? "OK" : "FAIL") << endl << "Corrected" << endl;
